document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    const scaleFactorX =
      ((e.clientX + e.clientY) / (window.innerWidth + window.innerHeight)) * 20;
    const scaleFactorY =
      ((e.clientX + e.clientY) / (window.innerWidth + window.innerHeight)) * 20;
  ...
  function updateTimeText() {
    ...
    setStylesBasedonColor(randomColor);
    ...
    if (isBlackBackground) {
      ...
      if (
        (animationTime >= 0.0 && animationTime < 0.3) ||
        (animationTime >= 0.45 && animationTime < 0.55) ||
        (animationTime >= 0.7 && animationTime <= 1)
      ) {
  ...
  function timetoWords(time) {
    ...
    if (minutes === 0) {
      ...
      if (minutes === 1) {
        ...
        timeInWords += `${minuteWords[minutes]} MINUTES PAST ${
          hourWords[hours % 12]
        }`;
      } else {
        const tens = Math.floor(minutes / 10);
        const ones = minutes % 10;
        timeInWords += `${minuteWords[20 + tens]} ${
          minuteWords[ones]
        } MINUTES PAST ${hourWords[hours % 12]}`;
      }
    } else if (minutes > 30) {
      ...
      if (remainingMinutes === 1) {
        ...
        timeInWords += `${minuteWords[remainingMinutes]} MINUTES TO ${
          hourWords[(hours % 12) + 1]
        }`;
      } else {
        ...
        timeInWords += `${minuteWords[20 + tens]} ${
          minuteWords[ones]
        } MINUTES TO ${hourWords[(hours % 12) + 1]}`;
      }
    } else {
      timeInWords += `${minuteWords[minutes]} MINUTES PAST ${
        hourWords[hours % 12]
